...
struct server_service daemon = {.is_stopped = false};
...
{
    struct khttp_worker *work;

    if (!(work = kmalloc(sizeof(struct khttp_worker), GFP_KERNEL)))
        return NULL;
...
int http_server_daemon(void *arg)
{
    ...
    free_work();
    pr_info("http_server_daemon returns\n");
    return 0;
}
